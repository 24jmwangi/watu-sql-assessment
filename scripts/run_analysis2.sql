...
-- Question 2: Add a column with the number of loans per customer.
\echo ''
...
-- Question 3: Select specific bike models and extract engine size.
\echo ''
...
FROM vehicle
WHERE model_name ILIKE '%100CC%'
    OR model_name ILIKE '%125CC%'
    OR model_name ILIKE '%150CC%';

-- Question 4: Total principal amount per client full name and per vehicle make.
\echo ''
...
GROUP BY client_full_name, vehicle_make
ORDER BY total_principal_amount DESC;

-- Question 5: Add chronological loan order for each client.
\echo ''
